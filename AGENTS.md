...
- URL-style asset imports such as videos, raster images, and fonts automatically inject an idempotent runtime `<link rel="preload">`; this is implemented by `scripts/preload-assets.ts`. SVG imports are excluded because `scripts/svg-loader.ts` turns them into Vue components.
...
- Space shortcuts are wired in `setup/shortcuts.ts`: Space first calls `advanceActiveStage()` and falls back to `nav.next()`, while Shift+Space first calls `retreatActiveStage()` and falls back to `nav.prev()`.
- `advanceActiveStage()` and `retreatActiveStage()` operate on the currently active `useStage` controller and return `true` when they handled the request. Use them for autoplay or custom controls that should drive the active slide's stage.
- Set `stage.busy = true` while async transitions or media segments are running. Additional Left/Right requests are queued as a signed delta; set `stage.busy = false` when the animation can accept the next step.
