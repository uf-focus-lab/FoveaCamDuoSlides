...
- The **slide entrance (`0 → 1`) is uninterruptable**: on activation `useStage` holds `busy` until the slide-in transition settles (finite animations under `#slideshow`, since Slidev runs the transition on the `SlideWrapper` above `#slide-content`), so mashing keys can't skip past the initial view before it has animated in. A transient initial stage already owns `busy` and drives its own pass-through, so the entrance hold is skipped there.
