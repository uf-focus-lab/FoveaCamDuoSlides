...
- `presist` (misspelled in the implementation) is an option on the config object that controls stage persistence across navigation: `true` always restores the last stage on re-entry; `false` always restarts at stage `1` (and resets to `0` once the slide is hidden); `"back"` (the **default**) restores only when entering the slide backward from a later slide, and restarts at `1` on a forward entry. Entry direction is read from the `currentSlideNo` watch's own `to`/`from` (backward = `to < from`), **not** Slidev's shared `navDirection` ref — that ref lags one navigation behind by the time this watcher runs, which would flip `back` persistence so a forward re-entry wrongly restored the saved stage.
...
- `advanceActiveStage(fallback?)` and `retreatActiveStage(fallback?)` operate on the currently active `useStage` controller and return `true` when they handled the request (this includes being dropped while busy — see below). The optional fallback runs only when a non-busy request reaches beyond the last/first stage. Use these helpers for autoplay or custom controls that should drive the active slide's stage.
- Set `stage.busy = true` while async transitions or media segments are running, then `false` when the animation can accept the next step. A transient stage holds `busy` **synchronously by default** (its stage watch is `flush: "sync"`), so it is busy the instant it is entered. **Left/Right/Space input that arrives while `busy` is `true` is dropped, not queued** — this stops a rapid keypress from skipping past a transient stage mid-animation.
- The **slide entrance (`0 → 1`) is uninterruptable**: `busy` is held from the *start* of the slide switch (activation) until the slide-in transition *ends* (finite animations under `#slideshow`, since Slidev runs the transition on the `SlideWrapper` above `#slide-content`), so mashing keys can't skip past the initial view before it has animated in. The settle promise is captured as `entering` at activation; a **transient initial stage** owns `busy` itself but its hold `await entering` first, so a transient stage `1`'s `1 → 2` pass-through does not fire until the slide has finished entering. Mid-slide transients see an already-resolved `entering`, so it is a no-op for them.
...
- **Shim, not the app shell.** The slide skips the standalone `index.vue` shell (header, popups, self-reload HMR guard) and composes the pieces itself: `pages/07` owns the one `new Simulation("mission")`, calls `Dialog.provide(sim)` to populate the dialog registry, and renders the equations plus the simulation frame and controls as normal (unconditional) child components. `pages/07C` renders the world canvas (`src/simulation.panel.vue`, wrapped by the themed `.crypsis-app` host in `07B`) on the right half; `pages/07D` renders the submodule's `<Dialogs>` list (simulation controls + per-entity dialogs) as an overlay on the **left half** so it never covers the simulation, behind an invisible fold button (opaque only on hover). **Import submodule files from a deck file by RELATIVE path** (`../crypsis-simulation/…`) — bare `lib/`/`src/` only resolve for importers *inside* the submodule.
